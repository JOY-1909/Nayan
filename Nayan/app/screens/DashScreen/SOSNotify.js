...
export default function SOSNotify() {
  ...
  const latlongtolocation = async (lat, lng) => {
    try {
      ...
      return { display_name: 'Location unavailable' };
    }
  }
  const sendEmergencySMS = async (location) => {
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        console.error('SMS backend returned non-JSON response (server may be offline)');
        return false;
      }
  ...
  const initiateSOSCalls = async () => {
    try {
      ...
      const contentType = response.headers.get('content-type');
      if (!contentType || !contentType.includes('application/json')) {
        console.error('Call backend returned non-JSON response (server may be offline)');
        return false;
      }
  ...
  const sendEmergencyAlert = async (location) => {
    try {
      ...
      const contentType = response.headers.get('content-type');
      if (!contentType || !contentType.includes('application/json')) {
        console.error('Alert backend returned non-JSON response (server may be offline)');
        return;
      }
