  ...
  return (
    <div>
      ...
      >
        ...
        {/* Render Crime Data as Red Circle Markers (Unsafe Locations) */}
        {Array.isArray(crimeData) && crimeData.length > 0 && crimeData.slice(0, 500).map((crime, index) => (
          crime.latitude && crime.longitude ? (
            <CircleMarker
              ...
              eventHandlers={{
                click: () => onCrimeClick && onCrimeClick(crime),
              }}
            >
              <Popup>
                <strong style={{ color: 'red' }}>⚠️ Unsafe Location</strong>
                <br />
                <strong>Crime Type:</strong> {crime.crime_type || 'Unknown'}
                <br />
                <strong>Time:</strong> {crime.timestamp || 'N/A'}
                <br />
                <strong>City:</strong> {crime.city || 'N/A'}
                <br />
                <strong>Cluster:</strong> {crime.cluster || 'N/A'}
              </Popup>
            </CircleMarker>
          ) : null
        ))}

        {/* Render Filtered Centroids as Purple Markers */}
        {Array.isArray(filteredCentroids) && filteredCentroids.map((centroid, index) => (
          <CircleMarker
            key={`centroid-${index}`}
            center={[centroid.coordinates.latitude, centroid.coordinates.longitude]}
            radius={12}
            pathOptions={{
              color: '#9400D3',
              fillColor: '#9400D3',
          ...
          >
            <Popup>
              <strong style={{ color: '#9400D3' }}>🔥 Crime Hotspot Cluster {centroid.cluster}</strong>
              <br />
              High concentration of crimes in this area
            </Popup>
          </CircleMarker>
        ))}
