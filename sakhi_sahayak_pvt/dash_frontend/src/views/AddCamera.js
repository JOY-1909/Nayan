    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const cameraData = {
                ...
                latitude,
                longitude,
                link: `http://${ipAddress}/video`
            };

            const response = await fetch('http://127.0.0.1:8000/cameras/addCamera', {
                method: 'POST',
                headers: {
            ...
            if (result.success) {
                ...
                triggerToast('Failed to add camera: ' + result.message, 'error');
            }
        } catch (error) {
            console.error('Error adding camera: ', error);
            triggerToast('Error adding camera: ' + error.message, 'error');
        }
    };
