...
function AddVideoGesture() {
  ...
  const startCamera = async () => {
    try {
      // Use more specific constraints for better DroidCam compatibility
    ...
    websocketRef.current.onmessage = async (event) => {
      ...
      if (data.error) {
        ...
        if (data.inference[0] === "Wrist Formed Detected") {
          ...
          // Create a real alert in the backend
          try {
            await fetch('http://127.0.0.1:8000/alerts/create', {
              method: 'POST',
              headers: {
              ...
              body: JSON.stringify({
                type: 'Gesture Alert',
                message: `Wrist distress gesture detected - Help signal identified`,
                source: 'Video Gesture Analysis',
              }),
            });
